...
| CAMP-08 | Zonas de subida por catálogo, solo aceptan `.pdf` | Implementada | 3 | Ya no son 8 fijas: portadas (1 por catálogo) + instrucciones (una por idioma, cantidad abierta — se puede añadir cualquier idioma por nombre libre, sin lista cerrada en código) — cambio funcional solicitado. **Regresión visual corregida (2026-08-04)**: durante un bloque anterior estas zonas se habían implementado como `<input type="file">` nativo en vez de la zona de arrastrar/soltar con borde punteado del resto de la app — `CampanaFileDropZone` (`features/campanas/ui/campana-file-drop-zone.tsx`) réplica el aspecto exacto de `cover-zone-*`/`instr-zone-*` (index.html ~1329-1343, `setCoverFile()` ~4763-4772): estado vacío ("📄/📋 Arrastra el PDF o haz clic"), estado con archivo nuevo seleccionado (fondo/borde ámbar, nombre+tamaño) y estado con PDF ya guardado (fondo/borde verde, enlace "Ver" + "Sube otro para reemplazar") — sin perder la arquitectura nueva de idiomas dinámicos |
...
- **Nueva evidencia (2026-08-04) — ZIP de consultas SQL históricas del propietario del proyecto**: `creacion_de_usuarios.sql`, `display_name.sql`, `untitled_query_18.sql` y `conceder_permisos.sql` muestran que las altas de usuario reales en producción se han hecho históricamente por dos vías completamente manuales y ajenas al código de `index.html`: (a) `INSERT` directo en `auth.users` + `perfiles` desde el SQL Editor (altas iniciales), y (b) usuario creado a mano desde el Dashboard de Supabase → Authentication, seguido de un `INSERT ... ON CONFLICT (id) DO UPDATE` manual en `perfiles` (`untitled_query_18.sql`, comentario literal: "ejecutar después de crearlos en el panel"). Ninguna de las dos pasa por `saveUser()`/`confirmImport()` ni por ninguna Edge Function — son procedimientos de administración de base de datos, no comportamiento de la aplicación desplegada, así que no hay ningún mecanismo del original que replicar aquí (la pregunta "¿qué mecanismo usa el original?" no tiene una respuesta distinta de "ninguno del lado de la app"). Esto SÍ refuerza el riesgo ya documentado en `create-user/index.ts` (placeholder sin verificar contra el código real de producción): si el equipo nunca ha necesitado que la Edge Function funcione (porque siempre da de alta a mano), es plausible que nunca se haya probado de verdad.
- **Hallazgo adicional derivado del mismo ZIP**: `creacion_de_tablas.sql` (la primera versión del esquema, ya superada — su enum `rol_usuario` ni siquiera tiene las variantes de canal) define un trigger `handle_new_user()` sobre `auth.users` que autoinserta la fila de `perfiles` a partir de `raw_user_meta_data`. No hay evidencia de que este trigger exista todavía en el esquema real reconciliado en `03-modelo-datos.md` (nunca apareció en esa auditoría), y `untitled_query_05.sql` ("Reset completo") muestra un `DROP FUNCTION handle_new_user CASCADE` en algún punto de la historia del proyecto — pero tampoco hay evidencia de que NO exista hoy. Por si acaso: `create-user/index.ts` ahora hace `upsert` (con `onConflict: "id"`) en vez de `insert` al escribir `perfiles`, exactamente el patrón `ON CONFLICT (id) DO UPDATE` que ya usan los propios scripts SQL del equipo — así el código funciona igual si el trigger existe (evita un choque de clave primaria) que si no existe (se comporta como el insert de antes). También se pasa `user_metadata: {nombre, rol, codigo}` a `auth.admin.createUser()`, por si algún trigger equivalente lo consulta. **Sigue pendiente de verificación directa en producción** (`select proname from pg_proc where proname = 'handle_new_user';` en el SQL Editor) para saber con certeza si el trigger existe hoy — no bloquea nada mientras tanto, porque el upsert cubre ambos casos.

- **Persiste tras el fix (2026-08-04, reportado por el propietario del proyecto)**: mismo síntoma exacto ("0 creados, 124 errores", sin tráfico en Supabase, sin excepción en consola) DESPUÉS de aplicar la corrección anterior. Instrumentación completa añadida en `crearUsuario()` (Server Action), `confirmarImportacion()` (cliente) y `create-user/index.ts` (Edge Function) — ver comentarios "INSTRUMENTACIÓN TEMPORAL" en cada archivo; no retirar hasta confirmar la causa con los logs reales del propietario. **Hallazgo operativo crítico mientras tanto**: `create-user/index.ts` es una Edge Function que Supabase NO redespliega automáticamente al hacer `git push` — el código de este repositorio solo tiene efecto en producción si alguien copia el archivo entero en Dashboard → Edge Functions → create-user → editor y pulsa "Deploy" a mano (ver cabecera del propio archivo, ya documentado desde la Fase 0). Es decir: los cambios de esta migración a `create-user/index.ts` (incluidos el `upsert` y la instrumentación) **no están necesariamente desplegados hoy** en el proyecto Supabase real, aunque estén en el repositorio — la función que de verdad se ejecuta al invocar la importación puede seguir siendo una versión distinta (o inexistente). Esto por sí solo explicaría el síntoma sin necesitar ningún otro bug: si el endpoint no existe o difiere, cada llamada falla por un motivo que nunca llega a los logs de Supabase (la petición no llega a ejecutar ninguna consulta) ni a la consola del navegador (la excepción, si la hay, ocurre dentro del Server Action, que corre en el servidor de Next.js, no en el navegador — visible solo en la terminal de `next dev`/`next start` o en Vercel → Functions → Logs). **Antes de seguir buscando un bug de código nuevo, hay que confirmar**: (1) que `create-user/index.ts` está realmente desplegado en el Dashboard con el código actual del repositorio, y (2) revisar los logs del SERVIDOR de Next.js (no solo la consola del navegador ni los logs de Supabase) durante una importación real, ahora que la instrumentación de arriba los genera.

- **Segunda revisión exhaustiva (2026-08-04), a petición explícita del propietario del proyecto de no cerrar la investigación con la hipótesis de la Edge Function sin evidencia**: repasado de nuevo, campo a campo, todo el camino de datos desde el Excel hasta la respuesta interpretada por el cliente. No se ha encontrado ningún bug adicional de código en este repositorio (los tres archivos — `import-usuarios-modal.tsx`, `crear-usuario.action.ts`, `create-user/index.ts` — están limpios y consistentes entre sí: mismos nombres de campo, mismo orden, `rolValido()`/`VALID_IMPORT_ROLES` no bloquean el envío, solo lo señalan en la previsualización). Dos cosas SÍ hay que descartar con datos reales, no con lectura de código, porque **ninguna de las dos produciría una excepción visible en la consola del navegador ni aparecería en los logs de Supabase de la forma en que se ha buscado hasta ahora** — encajan igual de bien que la hipótesis de la Edge Function con el síntoma reportado:

  1. **Desajuste entre el `rol` del Excel y el enum real `rol_usuario` de Postgres** (`docs/03-modelo-datos.md` § 3.2, línea `rol rol_usuario not null default 'comercial'`). Si la columna 4 del Excel (posición fija, `r[3]`, documentada en el propio modal como "rol") contiene un valor que no es EXACTAMENTE uno de los 10 valores del enum (`admin, marketing, comercial, comercial_nacional, comercial_exportacion, responsable, responsable_nacional, responsable_exportacion, disenador, responsable_diseno`) — por ejemplo, la columna está vacía, tiene espacios, usa un nombre distinto, o las columnas del Excel real no están en el orden que se documenta en el modal ("email · nombre · contraseña · rol · código") — el `upsert` en `perfiles` de `create-user/index.ts` falla con un error de Postgres (`invalid input value for enum rol_usuario`) para ESA fila. Si el problema afecta a la columna en sí (no a un valor puntual), falla el 100% de las filas por igual, encajando con "0 creados, 124 errores": `auth.admin.createUser()` sí tendría éxito primero (nada en su validación depende del enum — `rol` viaja como texto libre dentro de `user_metadata`), así que se crearía un usuario en `auth.users` en cada fila, pero como el `perfiles.upsert()` posterior falla, la fila nunca aparece en la pantalla "Usuarios" (que lee de `perfiles`, no de `auth.users`) — de ahí "0 creados" aunque sí haya tráfico real contra Supabase. El error se devuelve como JSON con `status: 400` desde la Edge Function (no como una excepción no controlada), por lo que nunca se ve como "excepción en consola" en ningún sitio — aparece como texto en el campo `error` de la respuesta, que con la instrumentación actual sí queda registrado en los logs del servidor de Next.js y de la Edge Function, pero no como algo que salte a la vista si no se mira ese campo en concreto.
     **Comprobación decisiva, sin necesitar leer ningún log**: en el Dashboard de Supabase → Authentication → Users, ordenar por fecha de creación. Si tras el intento de importación aparecen ~124 usuarios nuevos con la hora del intento (aunque la app siga mostrando 0 en "Usuarios"), esto por sí solo confirma que el fallo ocurre específicamente en el paso de `perfiles.upsert()` dentro de la Edge Function — y con ello, muy probablemente en el valor de `rol` que trae el Excel real — y no antes (ni en el Server Action, ni en la propia llamada a Auth). Si en cambio NO aparece ningún usuario nuevo en Authentication → Users, el fallo es anterior a `auth.admin.createUser()` (validación en `crearUsuario()`, error de red, JWT rechazado por el gateway de la Edge Function, o la Edge Function ni siquiera se ejecuta) — lo que apunta de vuelta a las hipótesis de despliegue/instrumentación de más arriba.
  2. **Verificación de JWT en el gateway de Edge Functions**: por defecto, Supabase exige que toda invocación a una Edge Function lleve un JWT válido en `Authorization: Bearer <token>` (comprobado por la propia plataforma, antes de que el código de la función se ejecute) salvo que esa función tenga desactivada la verificación (`--no-verify-jwt` en CLI, o el toggle equivalente en Dashboard → Edge Functions → create-user → "Enforce JWT Verification"). `crearUsuario()` ya manda el `access_token` real de la sesión del usuario como Bearer — un JWT de usuario autenticado normal es válido para esa comprobación, así que en el caso esperado no debería rechazarlo; pero si esa función en particular tuviera la verificación desactivada esperando en su lugar comprobar el rol manualmente en el propio código (cosa que el `create-user/index.ts` de este repositorio NO hace — no hay ninguna comprobación de rol antes de llamar a `auth.admin.createUser`), un rechazo del gateway se vería como un `401` con un cuerpo que no es el JSON que espera `crearUsuario()` — cubierto por la rama `!res.ok` existente, así que también se contabilizaría como error sin excepción.

  **No hay forma de decidir entre estas hipótesis (incluida la de la Edge Function desplegada a mano) desde este entorno de trabajo — no tiene acceso al Dashboard de Supabase, a los logs de Vercel/`next start`, ni al proyecto real** — es una limitación de este entorno, no una decisión de dejar de investigar. El siguiente intento de importación con la instrumentación ya desplegada, revisando los tres sitios de log a la vez (consola del navegador, logs del servidor de Next.js/Vercel, logs de la Edge Function en el Dashboard) más la comprobación decisiva de Authentication → Users de arriba, debería bastar para identificar la línea exacta sin necesidad de una ronda adicional.

- **Cierre (2026-08-04)**: la comprobación decisiva confirmó usuarios ya creados en Authentication de intentos previos — reintentar la importación con esos mismos emails hacía fallar `auth.admin.createUser()` con "ya registrado" en cada fila, sin llegar nunca al `upsert` de `perfiles`. `create-user/index.ts` ahora detecta ese caso, localiza el usuario ya existente y continúa el upsert igualmente en vez de abortar. `ImportarUsuariosModal` muestra además el motivo real de cada fila fallida en el propio modal (antes solo un recuento). Pendiente el único paso que sigue siendo manual por diseño (docs/10): copiar este archivo en el Dashboard de Supabase y pulsar "Deploy".
...
### Segunda pasada (2026-08-04) — a petición del propietario del proyecto, sobre hallazgos ya reportados

- **"PORTADAS PERSONALIZADAS" en la topbar**: el `<span>` tenía un estilo inline (`color:white, fontWeight:700`) que pisaba por completo la regla `.topbar-brand span { color:var(--c-mid); font-weight:400 }` — esa regla ya existía, correcta, en `globals.css` desde antes; el bug estaba solo en el inline de `app-shell.tsx`. Corregido quitando el inline y dejando que la clase se aplique. Verificado con `getComputedStyle` contra una copia aislada del CSS+HTML original: `color`, `font-weight`, `font-size`, `letter-spacing` y los márgenes del separador "|" coinciden ahora exactamente (mismos valores computados en ambos, incluido el hover de "Mi cuenta"/"Cerrar sesión": `rgba(255,255,255,.12→.22)` y `.1→.2`).
- **Revisión píxel a píxel del resto de la topbar** (altura, padding, logos, separadores, tipografía, botones, selector de rol, campana de notificaciones): sin más diferencias objetivas — `.topbar`/`.topbar-brand` ya coincidían byte a byte con el original antes de esta pasada.
- **Campo "Fecha de cierre" (Campañas)**: causa real localizada — ni `index.html` ni la migración incluían nunca `input[type="date"]` en el selector compartido de estilos de formulario (`input[type="text"], input[type="email"], ...`), pero en `index.html` eso no se notaba porque no hay ningún reset global que despoje al navegador de su borde nativo; en la migración, el preflight de Tailwind sí lo hace (`border-width:0`, fondo transparente), dejando el campo completamente sin estilizar — más roto que en el original, no solo "igual de no-estilizado". Corregido añadiendo `input[type="date"]` a esa misma regla compartida en `globals.css`: mismo borde, alto, padding y radio que el resto de inputs del formulario, con el icono de calendario nativo del navegador ya alineado dentro de esa caja — sin necesidad de un componente de calendario nuevo.

## Diagnóstico de bloqueos funcionales (2026-08-04)

Tres flujos reportados como rotos por el propietario del proyecto. Diagnóstico exclusivamente — **sin aplicar ningún cambio de código a los puntos 1 y 2**, a petición explícita ("no implementes todavía nada"). El punto 3 solo incorpora instrumentación de diagnóstico (ver más abajo), no una corrección.

### 1. Creación de campañas — error 413 al subir los PDFs

**Causa exacta, localizada a nivel de código fuente de Next.js (no es una hipótesis)**: `saveCampana()` (`features/campanas/application/save-campana.action.ts`) es un Server Action invocado desde `<form action={formAction}>` (`campana-form.tsx`) — el formulario envía TODOS los PDF seleccionados (una portada por catálogo + una instrucción por catálogo y por idioma) como campos de un único `FormData`, en una sola petición `multipart/form-data`.

Next.js impone un límite de tamaño de body a los Server Actions **antes** de que la función de la acción llegue a ejecutarse — es decir, antes de que `saveCampana()` reciba el control, y por tanto antes de cualquier llamada a Supabase. El límite se define y se comprueba en el propio paquete `next` instalado en este proyecto:

- `node_modules/next/dist/server/app-render/action-handler.js:478-480`: `const defaultBodySizeLimit = '1 MB'` → `bodySizeLimitBytes = 1024 * 1024` (1 MiB exacto) — se usa este valor por defecto porque `next.config.ts` no define `serverActions.bodySizeLimit`.
- Línea 622-627 (ruta Node.js, la que usa esta app — no hay ningún `export const runtime = "edge"` en todo `src/`): un `Transform` de streaming (`sizeLimitTransform`) cuenta los bytes acumulados de la petición a medida que llegan; en el instante en que superan `bodySizeLimitBytes` lanza `new ApiError(413, "Body exceeded 1 MB limit...")`.
- Línea 652: además, el parser multipart (`busboy`) se configura con `limits: { fieldSize: bodySizeLimitBytes }` — el mismo límite se aplica también por campo individual (un solo PDF ya grande dispara el mismo 413 aunque el resto del formulario esté vacío).

El mensaje de consola que ve el propietario ("An unexpected response was received from la server.") sale de una capa distinta, también dentro de Next.js: `node_modules/next/dist/client/components/router-reducer/reducers/server-action-reducer.js:106` — el cliente exige que una respuesta de Server Action válida tenga `content-type: text/x-component`; la respuesta 413 no lo tiene (es la página de error genérica de Next), así que el cliente descarta el mensaje real ("Body exceeded...") y lanza este genérico en su lugar. Es la misma pieza de código para cualquier error de Server Action que no venga en el formato esperado — no es un error personalizado de esta aplicación.

**Conclusión sobre la capa responsable**: es exclusivamente Next.js (el propio framework), en el proceso del servidor (Node.js runtime), antes de invocar la acción. No es Vercel (su límite de payload a nivel de plataforma es independiente y no se llega a evaluar, porque Next.js corta antes), no es un Route Handler (no se usa ninguno aquí), no es Edge Runtime (no se usa en esta app) y no es Supabase (nunca se llega a hacer ninguna petición a Supabase). Encaja también con por qué es tan reproducible: un PDF a 300dpi de una sola página ya puede rondar o superar 1MB; con 4 catálogos × 1 portada + N idiomas de instrucciones, superarlo es lo normal, no un caso límite.

**Opciones de arquitectura (comparativa, sin implementar)**:

1. **Aumentar `serverActions.bodySizeLimit` en `next.config.ts`** (p. ej. a `'25mb'`). Es el cambio más pequeño (una línea de configuración), pero no resuelve el problema de fondo: cada PDF sigue viajando dos veces por la red (navegador → servidor Next.js → Supabase Storage) en vez de una, consume memoria del servidor por petición (todo el body se buferiza/transforma en el proceso de Node antes de llegar a `saveCampana()`), y solo desplaza el límite, no lo elimina — con suficientes idiomas/catálogos se puede volver a superar cualquier valor fijo que se elija. Tampoco hay garantía de que el límite de la plataforma de despliegue (Vercel u otra) no imponga su propio techo por debajo del valor elegido.
2. **Subir los archivos directamente a Supabase Storage desde el navegador (client-side), y que el Server Action solo reciba las rutas/URLs ya subidas** — el mismo patrón que ya usa `index.html` (`uploadCoverFiles()`, `~4775-4805`: sube con el token de sesión del usuario directamente a Storage, luego guarda solo la URL pública en la fila de `campanas`). El Server Action deja de recibir ningún `File`, solo texto (nombre, descripción, catálogos, y un objeto con las URLs ya subidas) — un payload de unos pocos KB, muy por debajo de cualquier límite. Requiere que el cliente use el mismo `createClient()` de `shared/infrastructure/supabase/browser-client.ts` para llamar a `.storage.from(...).upload()` antes de enviar el formulario (o al cambiar cada archivo), en vez de mandar el `File` dentro del `FormData` del Server Action.
3. **Subida a Storage vía un Route Handler dedicado** (p. ej. `app/api/campanas/upload/route.ts`) en vez de directamente desde el cliente. Mismo resultado que la opción 2 (el Server Action solo ve URLs), pero con un paso intermedio por el servidor — solo aportaría algo si hiciera falta validar/transformar el archivo en el servidor antes de subirlo (no es el caso aquí: son PDFs que se suben tal cual). Innecesariamente más complejo que la opción 2 para este caso.

**Resuelto (2026-08-04)** — el propietario del proyecto descartó explícitamente la opción 1 ("Descarto aumentar `serverActions.bodySizeLimit`") y pidió implementar la opción 2 en toda la aplicación, con una única infraestructura compartida. Ver "## Arquitectura de subida de archivos — resuelto (2026-08-04)" más abajo para el detalle de la implementación.

### 2. Riesgo compartido — carga masiva de portadas (Diseño) y otras subidas

**Mismo mecanismo, mismo límite, mismo Server Action fetch-action**: `procesarCargaMasiva(formData)` (`features/diseno/application/procesar-carga-masiva.action.ts`) también es un Server Action con `"use server"`; se invoca desde `carga-masiva-modal.tsx` con `await procesarCargaMasiva(fd)`, no desde un `<form>`, pero **la vía de invocación no cambia nada**: una llamada directa a una función de servidor desde código de cliente pasa por la misma ruta `isFetchAction && isMultipartAction` de `action-handler.js`, con el mismo `bodySizeLimitBytes` de 1 MB por defecto. La carga masiva permite seleccionar N archivos (uno por cliente/SAP) en una sola tanda — el propio propósito de la función es subir varios a la vez, así que superar 1MB acumulado es, si acaso, más fácil aquí que en Campañas, no menos.

**Conclusión**: no es un caso aislado — es un problema arquitectónico compartido por **cualquier Server Action que reciba archivos directamente en su `FormData`**, no solo por los dos flujos que el propietario preguntó. La misma exposición existe también en:
- `saveSolicitud()` (`features/solicitudes/application/save-solicitud.action.ts`) — logo del cliente (múltiple) + diseño propio por catálogo (múltiple).
- `marcarDisenoListo()` (`features/solicitudes/application/detalle-actions.ts`) — subida de diseño final desde el detalle de una solicitud.

Ninguno de estos había fallado todavía en este entorno (los archivos de prueba usados hasta ahora son pequeños), pero **la condición que provoca el fallo en Campañas estaba igualmente presente en los tres**. Es un riesgo real, no solo teórico, en cuanto se usen archivos de tamaño de producción.

**Resuelto (2026-08-04)** — los cuatro puntos (Campañas, `saveSolicitud`, `marcarDisenoListo`/`solicitarModificacion`, carga masiva) se migraron a la vez al mismo patrón de subida directa a Storage. Ver la sección siguiente.

## Arquitectura de subida de archivos — resuelto (2026-08-04)

A petición explícita del propietario del proyecto: *"Todos los archivos deberán subirse directamente desde el navegador a Supabase Storage. Los Server Actions únicamente recibirán: rutas de los archivos; nombres; metadatos. Nunca los binarios."*, aplicado a toda la aplicación con una única infraestructura compartida (sin duplicar la lógica de subida cuatro veces).

**Infraestructura compartida — `webapp/src/shared/storage/`**:
- `constants.ts` — `STORAGE_BUCKET = "portadas-adjuntos"` (antes duplicado como constante local en 4 Server Actions distintos).
- `types.ts` — `UploadedFile` (`path`, `url`, `nombre`, `tipo`, `size`): la única forma en la que un Server Action ve un archivo a partir de ahora.
- `upload-client.ts` (`"use client"`) — `subirArchivo(file, carpeta)`: única función de subida de toda la aplicación. Usa `shared/infrastructure/supabase/browser-client.ts` (mismo cliente anon-key + JWT de sesión que ya usaban los Server Actions para subir — mismas políticas RLS de `storage.objects`, por lo que no cambian los permisos de quién puede subir qué). También expone `borrarArchivoSubido(path)` para limpieza cuando el usuario quita un archivo ya subido antes de guardar.
- `server.ts` — `borrarArchivosStorage(supabase, paths)`: limpieza server-side best-effort para archivos subidos que un Server Action decide no usar (caso de la carga masiva: se sube antes de saber si el nombre coincide con alguna solicitud).

**Los cuatro flujos migrados, todos con el mismo patrón (subir al elegir el archivo → el formulario solo transporta metadata → el Server Action solo hace inserts/updates en BD)**:

1. **Campañas** (`campana-file-drop-zone.tsx` + `save-campana.action.ts`): cada zona de PDF (portadas e instrucciones, por catálogo e idioma) sube en cuanto se elige el archivo y sincroniza tres `<input type="hidden">` (`${name}_path`, `${name}_url`, `${name}_nombre`) para que el `<form action={saveCampana}>` los transporte como texto. `saveCampana()` ya no llama a `.storage.upload()` — solo lee esos campos y actualiza `covers`/`covers_instrucciones` con las URLs recibidas.
2. **Solicitudes** (`file-drop-zone.tsx` + `save-solicitud.action.ts`): cada archivo (logo del cliente, diseño propio por catálogo) se sube al añadirlo; el componente mantiene un input oculto por zona con un JSON-array de `UploadedFile[]` ya subidos. `saveSolicitud()` parsea ese JSON e inserta directamente en `adjuntos`/`logs`, sin tocar Storage.
3. **Detalle de solicitud** (`solicitud-detalle-modal.tsx` + `detalle-actions.ts`): "Diseño listo" (`marcarDisenoListo`) y "Solicitar modificación" (`solicitarModificacion`) ya no reciben un `FormData` con `File`s — reciben directamente un array/objeto `UploadedFile` (no hace falta ni `FormData`, porque estas dos acciones no se invocan desde un `<form>` nativo sino con `onClick`). El componente sube cada archivo nada más elegirlo y solo pasa la metadata al llamar a la acción.
4. **Carga masiva** (`carga-masiva-modal.tsx` + `procesar-carga-masiva.action.ts`) — el caso más delicado de los cuatro: el emparejamiento archivo→solicitud/catálogo depende del estado real de la BD en el momento de procesar (no del estado que el cliente tenía en memoria al abrir el modal), así que no se puede subir el archivo ya a su ruta final (`${solId}/diseno/...`) porque `solId` todavía no se conoce en el navegador. Solución: cada archivo se sube a una ruta de **staging** (`diseno/carga-masiva/...`) independiente de cualquier ID de solicitud, y solo se envía su metadata (`UploadedFile[]`) a `procesarCargaMasiva()`. El servidor sigue recalculando el emparejamiento contra la BD real exactamente igual que antes (`matchCargaFile()`); los archivos cuyo nombre no encuentra solicitud/catálogo destino se borran de Storage con `borrarArchivosStorage()` en vez de quedar huérfanos.

**Por qué no hacía falta mover/renombrar archivos tras crear la entidad**: la ruta física dentro del bucket es solo para organización — nada en la aplicación depende de que el path incluya el ID de la solicitud/campaña (los tres primeros flujos ya guardan la URL pública directamente en la fila de BD correspondiente, igual que antes). Evitar depender del ID en la ruta de subida es lo que permite subir el archivo ANTES de que exista la fila en BD, sin necesidad de un paso adicional de "mover" el archivo después.

**Indicador de progreso**: como la subida ya no ocurre al enviar el formulario (ocurre antes, al elegir el archivo), el indicador "⏳ Subiendo archivos..." y la deshabilitación del botón de envío ahora se basan en un estado `subiendo` que cada zona de subida reporta a su formulario padre (`onUploadingChange`), no en el estado `pending` del `useActionState` del Server Action.

### 3. Importación masiva de usuarios — instrumentación añadida, diagnóstico en curso

Ver H-01 más arriba para el detalle completo. Resumen: la corrección anterior (quitar las validaciones de contraseña/código del Server Action) sigue siendo correcta y necesaria, pero el propietario reporta el mismo síntoma después de aplicarla. Se ha añadido instrumentación exhaustiva (parámetros de cada fila, petición completa, respuesta completa, stack de cualquier excepción) en los tres puntos del flujo que SÍ están en este repositorio: `import-usuarios-modal.tsx` (cliente), `crear-usuario.action.ts` (Server Action, servidor de Next.js) y `create-user/index.ts` (Edge Function, solo tiene efecto si se redespliega a mano en el Dashboard de Supabase). **No se retira esta instrumentación hasta confirmar la causa** con los logs reales de una importación de prueba — revisando los tres sitios donde puede aparecer cada log (consola del navegador, logs del servidor de Next.js, y logs de la Edge Function en Supabase), no solo uno de ellos.
