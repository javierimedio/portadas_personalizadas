...
| NAV-14 | Logos de marca en topbar | Validada | 1 | Roly, Roly WRK, Stamina (mismas URLs externas que el original) + logo GOR Factory (ahora `public/images/GORFACTORY_LOGO.png`, subido al repo — antes se usaba `Logo_GOR.png` de `dev.gorfactory.com`, misma marca) — ocultos en móvil (`hidden md:flex`) igual que el original oculta elementos no esenciales de la topbar en `@media (max-width: 768px)` |
