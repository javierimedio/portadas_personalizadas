...
- **Edge Functions**: se conserva `create-user` (alta de usuario con Service Role Key), sin cambios. No se añaden funciones nuevas en esta migración — el envío real de email queda en `07-propuestas-futuras.md` § 4. **Nota (2026-08-04, `09-matriz-paridad-funcional.md` § H-01)**: el histórico de consultas SQL del propietario del proyecto muestra que las altas de usuario reales en producción se han hecho siempre a mano (Dashboard de Supabase + SQL manual en `perfiles`), nunca a través de esta Edge Function — así que "sin cambios" no debe leerse como "verificado en producción", sigue siendo la copia placeholder documentada en `webapp/supabase/functions/create-user/index.ts`.
...
- **Migraciones**: el SQL se ejecuta desde el SQL Editor del Dashboard de Supabase (ver `00-resumen-ejecutivo.md` § "Principio de trabajo" — se prioriza Dashboard sobre CLI) y se guarda como archivo versionado en el repo (`webapp/supabase/migrations`) para mantener el historial, aplicado siempre primero contra el proyecto Supabase de **desarrollo** — **partiendo de un clon del esquema real** (leído por SQL Editor sobre producción como primer paso de la Fase 0, ver `03-modelo-datos.md` § 3.1). Solo se aplican contra producción como parte del Cutover, nunca antes.
