...
### 4 · Desactivar la verificación JWT de la gateway

⚠️ **Paso obligatorio — sin él la función nunca ejecuta.** Por defecto la API Gateway de Supabase valida que el `Authorization: Bearer` contenga un JWT firmado por el proyecto antes de reenviar la petición al handler de Deno. Como `CRON_SECRET` es una cadena hexadecimal arbitraria (no un JWT), la gateway rechaza cada invocación del cron con `{"code":"UNAUTHORIZED_INVALID_JWT_FORMAT","message":"Invalid JWT"}` y el handler de Deno no llega a ejecutarse — no aparece ningún log ni en Runtime Logs ni en los Edge Logs porque la petición se corta en la capa de red antes de arrancar el proceso Deno.

Dashboard → Edge Functions → `send-notifications` → (icono de ajustes o pestaña Settings) → desactivar **"Verify JWT with legacy secret"** → guardar.

Con esto la gateway pasa la petición al handler sin validar el formato del Bearer token. La autenticación real corre a cargo de la comparación con `CRON_SECRET` en el propio código.

### 5 · Habilitar extensiones y programar el cron
...
Para verificar que pg_net está haciendo la llamada y qué respuesta recibe (útil en diagnóstico): `select id, created, status_code, content from net._http_response order by created desc limit 10;`.
