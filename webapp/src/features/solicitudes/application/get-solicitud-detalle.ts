...
export type DetalleAdjunto = { id: string; nombre: string; tipo: string; url: string; storage_path: string | null; subido_por: string | null; subido_por_nombre: string | null; created_at: string };
...
export async function getSolicitudDetalle(solicitudId: string, rolEfectivo?: string | null): Promise<SolicitudDetalle | null> {
  ...
  const [{ data: campana }, { data: catalogosRaw }, { data: adjuntos }, { data: logs }, { data: comercial }, { data: disenador }] =
    await Promise.all([
      ...
      supabase.from("adjuntos").select("id, nombre, tipo, url, storage_path, subido_por, subido_por_nombre, created_at").eq("solicitud_id", solicitudId).order("created_at"),
